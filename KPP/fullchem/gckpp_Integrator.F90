...
!~~~>  Interval (run-length) encoding of the auto-reduce maps: maximal
!      stretches of consecutive active species (SPC_MAP) and of consecutive
!      kept Jacobian slots (JVS_MAP), in full-space indices. Built once per
!      reduction by ros_yIntegrator; host-associated so ros_cPrepareMatrix
!      gathers by interval. useJvsRuns gates the interval path so the
!      append variant (ros_yIntegratorA) is unaffected.
   INTEGER       :: nSpcRuns, nJvsRuns
   INTEGER       :: SPC_RLO(NVAR), SPC_RHI(NVAR)
   INTEGER       :: JVS_RLO(LU_NONZERO), JVS_RHI(LU_NONZERO)
   LOGICAL       :: useJvsRuns
!~~~>   Parameters
...
!      Inline local parameters for AR.
   ...
   INTEGER :: irun, ilen, icmp ! interval-map loop counters
!~~~>  Initial preparations
   DO_SLV  = .true.
   DO_FUN  = .true.
   DO_JVS  = .true.
...
!~~~>  Parse species for reduced computation
   if (.not. reduced) then
     ...
     ! Run-length encode the maps into maximal consecutive intervals, so the
     ! gathers/scatters/stage updates below become stride-1 slice operations.
     ! SPC_MAP and JVS_MAP are strictly increasing by construction.
     nSpcRuns = 0
     DO i = 1,rNVAR
        IF (nSpcRuns .gt. 0) THEN
           IF (SPC_MAP(i) .eq. SPC_RHI(nSpcRuns)+1) THEN
              SPC_RHI(nSpcRuns) = SPC_MAP(i)
              CYCLE
           ENDIF
        ENDIF
        nSpcRuns = nSpcRuns+1
        SPC_RLO(nSpcRuns) = SPC_MAP(i)
        SPC_RHI(nSpcRuns) = SPC_MAP(i)
     ENDDO
     nJvsRuns = 0
     DO i = 1,cNONZERO
        IF (nJvsRuns .gt. 0) THEN
           IF (JVS_MAP(i) .eq. JVS_RHI(nJvsRuns)+1) THEN
              JVS_RHI(nJvsRuns) = JVS_MAP(i)
              CYCLE
           ENDIF
        ENDIF
        nJvsRuns = nJvsRuns+1
        JVS_RLO(nJvsRuns) = JVS_MAP(i)
        JVS_RHI(nJvsRuns) = JVS_MAP(i)
     ENDDO
     useJvsRuns = .true.
...
UntilAccepted: DO
   ...
   ! map cNONZERO back to full space, interval-wise: contiguous slice
   ! copies replace the indexed scatter (which serialized on aliasing).
   icmp = 1
   DO irun = 1, nJvsRuns
      ilen = JVS_RHI(irun) - JVS_RLO(irun)
      Ghimj(JVS_RLO(irun):JVS_RHI(irun)) = cGhimj(icmp:icmp+ilen)
      icmp = icmp + ilen + 1
   ENDDO
...
Stage: DO istage = 1, ros_S
      ...
      ! For the 1st istage the function has been computed previously
       ...
       ELSEIF ( ros_NewF(istage) ) THEN
         ...
         DO j = 1, istage-1
            ...
            DO irun = 1,nSpcRuns
               DO i = SPC_RLO(irun),SPC_RHI(irun)
                  Ynew(i) = Ynew(i) + alpha_factor * K(N*(j-1)+i)
               ENDDO
            ENDDO
         END DO
         Tau = T + ros_Alpha(istage)*Direction*H
      ...
      IF ((.NOT. Autonomous).AND.(ros_Gamma(istage).NE.ZERO)) THEN
         ...
         DO irun = 1,nSpcRuns
            DO i = SPC_RLO(irun),SPC_RHI(irun)
               K(ioffset+i) = K(ioffset+i) + HG * dFdT(i)
            ENDDO
         ENDDO
      ENDIF
...
! roll the new solution and error estimation into one loop.
   ...
   DO j = 1, ros_S
      DO irun = 1,nSpcRuns
         DO i = SPC_RLO(irun),SPC_RHI(irun)
            Ynew(i) = Ynew(i) + ros_M(j) * K(N*(j-1)+i)
            Yerr(i) = Yerr(i) + ros_E(j) * K(N*(j-1)+i)
         ENDDO
      ENDDO
   ENDDO
   Err = ros_ErrorNorm ( Y, Ynew, Yerr, AbsTol, RelTol, VectorTol )
...
#else
     IF (useJvsRuns) THEN
        ! interval-wise gather: kept Jacobian slots are contiguous runs
        ! (runs are host-associated from ros_yIntegrator; the append
        ! variant does not build runs and takes the indexed gather below)
        icmp = 1
        DO irun = 1, nJvsRuns
           ilen = JVS_RHI(irun) - JVS_RLO(irun)
           Ghimj(icmp:icmp+ilen) = -Jac0(JVS_RLO(irun):JVS_RHI(irun))
           icmp = icmp + ilen + 1
        ENDDO
     ELSE
